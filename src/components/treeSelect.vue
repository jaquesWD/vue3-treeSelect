<template>
  <el-select ref="select" v-model="selectValue" :multiple="multiple" collapse-tags :filter-method="dataFilter" @remove-tag="removeTag"
    @clear="clearAll" popper-class="t-tree-select" :style="{ width: width || '100%' }" v-bind="attrs">
    <el-option v-model="selectTree" class="option-style" disabled>
      ...
      <el-tree :data="options" :props="treeProps" class="tree-style" ref="treeNode" :show-checkbox="multiple"
        :node-key="treeProps.value" :filter-node-method="filterNode" :default-checked-keys="defaultValue"
        :current-node-key="currentKey" @node-click="handleTreeClick" @check-change="handleNodeChange" v-bind="treeAttrs"
        ></el-tree>
    </el-option>
  </el-select>
</template>
...
<script setup>
import { defineProps, ref, computed, watch, nextTick, onMounted, defineEmits } from 'vue'
...
const removeTag = (tagName) => {
  ...
  console.log(setlist);
  setlist = setlist.filter(v => v[VALUE_NAME.value] === selectTreeValue.find(item => item === v[VALUE_NAME.value]))
